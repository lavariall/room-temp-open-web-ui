...
@pytest.fixture(name="mock_tools_fixture")
def fixture_mock_tools():
    """Mock the Tools class."""
    with patch('src.web_app.tools') as mock:
        mock.get_room_climate = AsyncMock()
        yield mock

def test_read_root_success(mock_tools_fixture):
    """Test the root endpoint renders correctly with data."""
    # Mock the tools.get_room_climate to return valid JSON-like strings
    mock_tools_fixture.get_room_climate.side_effect = [
...
def test_chat_endpoint_no_tool():
    """Test chat endpoint without tool invocation."""
...
def test_chat_endpoint_with_tool(mock_tools_fixture):
    """Test chat endpoint with tool invocation."""
    ...
    with patch('src.web_app.ollama.chat') as mock_ollama:
        ...
        mock_tools_fixture.get_room_climate.return_value = (
            'Climate data for room_91: {"temperature": 22.5, "humidity": 45}'
        )
        ...
        mock_tools_fixture.get_room_climate.assert_called_with('room_91')
